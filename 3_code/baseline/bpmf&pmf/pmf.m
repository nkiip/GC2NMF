function [result, w1_P1, w1_M1] = pmf(train_vec, probe_vec, genes, mps)
rand('state',0); %set rand seed  
randn('state',0); %set rand see;

%% set parameters
epsilon = 50; % Learning rate 
lambda  = 1;%0.01; % Regularization parameter 
momentum = 0.8; 
...
N = 2710; % number training triplets per batch _set to 500;
...
%% Result
result = w1_P1 * w1_M1' + mean_rating;
